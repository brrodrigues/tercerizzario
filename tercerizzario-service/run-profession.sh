...
if [ "${PID}x" = "x" ]; then
  echo "DO NOT EXIST ${APPLICATION} APPLICATION SERVER RUNNING" 
else
  echo "KILLING THE PROCESS ${PID} REFERENCE ${APPLICATION_NAME} APPLICATION SERVER"
  kill -9 ${PID}
  echo "DONE"
fi
...
echo "STARTING ${APPLICATION_NAME} SERVICER..."

mvn spring-boot:run >> ${LOG_APPLICATION} &

sleep 1

echo "TYPE tail -f -n1000 ${APPLICATION_NAME}/${LOG_APPLICATION} AT TERMINAL"
